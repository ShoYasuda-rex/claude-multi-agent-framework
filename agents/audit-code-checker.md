...
Phase 1 は全Phaseの前提情報を収集するため、最初に単独で実行する。
...
Phase 1 完了後、結果（技術スタック、エントリーポイント一覧、Docker環境情報）をまとめ、ステージ2の各サブエージェントに渡す。

---

### ステージ2: 並列チェック（Agent ツールで同時実行）

Phase 1 の結果を prompt に含めて、以下の **5つのサブエージェント** を `run_in_background: true` で **1つのメッセージ内で同時に** 起動する。

| サブエージェント | 担当Phase | 内容 |
|----------------|-----------|------|
| **checker-A** | Phase 2 + 3 | 未使用ファイル検出 + デッドコード検出 |
| **checker-B** | Phase 4 + 5 | 依存関係チェック + 構造一貫性チェック |
| **checker-C** | Phase 7 + 9 | 品質チェック + リントチェック |
| **checker-D** | Phase 8 | リクエストフロー整合性 |
| **checker-E** | Phase 10 | 効率チェック |

各サブエージェントの prompt には以下を含めること：
- Phase 1 の検出結果（技術スタック、エントリーポイント、Docker環境）
- 担当Phaseの定義（下記の各Phase詳細をそのまま含める）
- 出力形式: 担当セクションの結果を構造化テキストで返す（ファイル出力不要）
- 重要ルール（下記「重要ルール」セクション参照）

Phase 6（セキュリティ）は audit-security-checker が担当するためスキップ。

---

### ステージ3: 結果の集約とレポート出力

全サブエージェントの完了を待ち、返ってきた結果を統合して `check_log/YYYY-MM-DD_HHMM_full_check.md` にレポートを出力する。
...
- **肥大化したコード**:
  ...
  - 行数とともに上位の違反箇所を報告する
- **キャッシュバスティング未設定**:
  - HTMLファイル内の `<script src>` / `<link href>` でローカルJS/CSSを参照している箇所に、バージョンパラメータ（`?v=`）またはハッシュ付きファイル名がない場合に検出する
  - ビルドツール（Vite, Webpack等）がハッシュ付きファイル名を自動生成する構成の場合はスキップする
  - CDN参照（外部URL）は対象外
...
### Phase 10: 効率チェック
プロジェクト全体のソースコードを対象に、パフォーマンス・効率面の問題を検出する。

- **N+1 問題**: ループ内での DB クエリ・API 呼び出し
  - ORM のリレーション呼び出し（`belongs_to`, `has_many` 等）がループ内で遅延ロードされている箇所
  - eager loading（`includes`, `preload`, `joinedLoad` 等）で解決可能かを併記する
- **ループ内の重い処理**: ループ内でのファイル I/O、HTTP リクエスト、正規表現のコンパイル、DOM 操作等
  - ループ外に移動可能な処理を特定する
- **不要なデータコピー**: 巨大な配列・オブジェクトの全体コピー（`Array.from`, `[...arr]`, `Object.assign`, `.dup`, `.clone` 等）が不要な箇所
  - 参照で十分な場合、またはストリーム処理に置き換え可能な場合を報告する
- **キャッシュ不足**: 同一引数で繰り返し呼ばれる重い計算・DB クエリ・API 呼び出し
  - メモ化、キャッシュ層の導入、計算結果の再利用で改善可能な箇所を特定する
- **非効率なアルゴリズム・データ構造**: 明らかに計算量を改善できる箇所
  - 例: 配列の線形探索を繰り返す → Set/Map に置き換え可能
  - 例: ネストしたループで全組み合わせを走査 → ソート+二分探索やハッシュで改善可能
- **不要な同期処理**: 並列実行可能な非同期処理を直列で実行している箇所
  - `await` の連続で独立した処理を順番に待っている → `Promise.all` 等で並列化可能
...
## 8. 効率チェック
### 8.1 N+1 問題
[ファイルパス:行 - クエリ/呼び出し内容 - 改善案]

### 8.2 ループ内の重い処理
[ファイルパス:行 - 処理内容 - ループ外への移動案]

### 8.3 不要なデータコピー
[ファイルパス:行 - コピー内容 - 改善案]

### 8.4 キャッシュ不足
[ファイルパス:行 - 繰り返し呼び出し内容 - キャッシュ/メモ化案]

### 8.5 非効率なアルゴリズム・データ構造
[ファイルパス:行 - 現状の計算量 - 改善案と改善後の計算量]

### 8.6 不要な同期処理
[ファイルパス:行 - 直列化されている処理 - 並列化案]

...
#### ステージ1: 文脈把握（直列・最初に実行）
Phase 1 を単独で実行する。全Phaseの前提情報を収集するため。

Phase 1 完了後、結果（プロジェクト文脈、技術スタック、推定コンテキスト）をまとめ、ステージ2の各サブエージェントに渡す。

#### ステージ2: 並列分析（Agent ツールで同時実行）
Phase 1 の結果を prompt に含めて、以下の **3つのサブエージェント** を `run_in_background: true` で **1つのメッセージ内で同時に** 起動する。

| サブエージェント | 担当Phase | 内容 |
|----------------|-----------|------|
| **arc-A** | Phase 2 | 技術選定の妥当性 |
| **arc-B** | Phase 3 + Phase 5 | アーキテクチャパターンの妥当性 + 組織適合性 |
| **arc-C** | Phase 4 + Phase 6 | 非機能要件の設計評価 + 進化可能性 |

各サブエージェントの prompt には以下を含めること：
- Phase 1 の検出結果（プロジェクト文脈、技術スタック、推定コンテキスト）
- 担当Phaseの定義（下記の各Phase詳細をそのまま含める）
- 出力形式: 担当セクションの結果を構造化テキストで返す（ファイル出力不要）
- 評価原則（下記「評価原則」セクション参照）

#### ステージ3: 総合評価とレポート出力
全サブエージェントの完了を待ち、返ってきた結果を統合して Phase 7（総合評価）を実施する。レポートを `check_log/YYYY-MM-DD_HHMM_architecture_check.md` に保存する。
...
#### 6.2 ゼロベース再設計
- **「今の知見でゼロから作り直すなら、どう設計するか？」** を問う
- 現在の設計と理想の設計のギャップを明示する
- ギャップが大きい箇所 = 構造レベルの負債（Phase 2-5 では検出しにくい）
- 「作り直す」こと自体を推奨するのではなく、**現設計のどこが最も理想から遠いか**を炙り出す道具として使う
...
> **Note**: Phase 7 はステージ3で実行する。ステージ2の全サブエージェントの結果を統合した上で総合判定を行う。
...
レポートを `check_log/YYYY-MM-DD_HHMM_architecture_check.md` に保存する。実際の現在日時を使用すること。`check_log` ディレクトリが存在しない場合は作成する。
...
### ゼロベース再設計
- **現設計との最大のギャップ**: ...（理想から最も遠い箇所を1〜3個）
- **再設計するなら変える点**: ...
- **現設計のまま進めてよい点**: ...（変えなくてよい理由も）

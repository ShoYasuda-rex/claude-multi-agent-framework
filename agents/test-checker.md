...
3. **同じモジュールのテストファイルあり** → そのファイルに不足テストケースを追加（既存の `it()` / `test()` は変更しない。ソースコードの変更で正しくなくなった場合のみ修正する）
4. **なし** → 新規生成
...
Phase 3 完了後、テスト生成+実行を **3つのサブエージェント** に分割し、`isolation: "worktree"` で **1つのメッセージ内で同時に** 起動する。

| サブエージェント | 担当 | 内容 |
|----------------|------|------|
| **test-A** | ユニットテスト | 生成 → 実行（Phase 5a） |
| **test-B** | インテグレーションテスト | 生成 → 実行（Phase 5a） |
| **test-C** | E2Eテスト | 生成 → 実行（Phase 5a） |

各サブエージェントの prompt には以下を含めること：
- Phase 1-3 の結果（対象ファイル、検知判定、既存テスト情報、テストフレームワーク）
- 担当層の生成方針（下記の各層詳細）
- ルール（下記「ルール」セクション）

**検知対象が1層のみの場合**（例: ユニットテストだけ必要）は並列化せず単独で実行する。

全サブエージェント完了後:
1. 各worktreeの変更をmainにマージ（衝突があれば解決）
2. Phase 5b（既存テスト実行）を実施
3. Phase 6（報告）を実施
...
- 同じモジュールのテストファイルが既にある場合は新規ファイルを作らず、既存ファイルに追記する
...
- **既存ファイル追記優先**: 同じモジュールのテストファイルが既にある場合、そのファイルにテストケースを追加する。新規ファイルは作らない。既存の `it()` / `test()` はソースコードの変更で正しくなくなった場合のみ修正する

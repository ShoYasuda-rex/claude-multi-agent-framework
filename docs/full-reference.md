...
→ プロジェクトルートに `CLAUDE.md` が生成され、初期状態がgitにコミットされる

### 3. 要件定義書から CORE/SPEC/ARCH を抽出
...
→ 要件定義書から必要な情報を抽出し、`docs/CORE.md`, `docs/SPEC.md`, `docs/ARCHITECTURE.md` を生成
...
| 動作・UI検証 | `.vc` |
| コード全体監査 | `.cod` |
| コード全体監査（Codex） | `.cod-dx` |
| セキュリティ監査 | `.sec` |
| セキュリティ監査（Codex） | `.sec-dx` |
| アーキテクチャ設計監査 | `.arc` |
| 並列実装 | `.tpi` |
...
**役割**: コードを書く前に「何を実装するか」を対話で整理。ファイル出力はしない。

**動作**:
1. docs/CORE.md があれば読み込み、プロダクトCoreを把握
2. docs/SPEC.md があれば読み込み、サービス仕様方針を把握
3. docs/ARCHITECTURE.md があれば読み込み、実装設計を把握
4. ヒアリング（何を実装？なぜ必要？完了条件？やらないこと？）
5. 整理・確認
6. EnterPlanMode に引き継ぎ
...
| `.cod-dx` | Codex MCP | コード全体監査（Codex CLI経由） | - |
| `.sec` | エージェント | セキュリティ監査 | user |
| `.sec-dx` | Codex MCP | セキュリティ監査（Codex CLI経由） | - |
...
| 破壊の種類 | .sc | .cc | .spc | .vc | .cod | .sec | .test |
|-----------|-----|-----|------|-----|------|------|-----|
| import/requireの破壊 | ✓ | ✓ | - | - | ✓ | - | - |
| CSSセレクタ不一致 | ✓ | ✓ | - | ✓ | - | - | - |
| 関数シグネチャ変更 | ✓ | ✓ | - | - | - | - | ✓ |
...
- 未使用ファイル・デッドコード検出
- 依存関係チェック（未インストール・未使用・循環参照・phantom依存）
- 構造一貫性（命名規則・重複コード）
- 品質チェック（デバッグ残骸・TODO・長大ファイル）
- リントチェック（biome, stylelint, rubocop, ruff）
- Guardテスト実行（`tests/guard/` が存在する場合）
...
### .cod-dx / .sec-dx（Codex MCP経由の監査）

Claude側の監査（`.cod`/`.sec`）と同じスコープを、Codex CLI（GPT-5.3-Codex）に実行させる。異なるモデルの視点で盲点を補完する目的。

- Claudeが `developer-instructions` でタスク内容・対象ファイル・設計方針・完了条件を自動編纂してCodexに渡す
- 同じディレクトリで作業するためコードは最初から共有
- Claude監査とは独立。手動で別々に呼ぶ
- 両方の結果を突き合わせることで、単一モデルの盲点を検出できる
...
| モデル | スキル |
|-------|-------|
| **opus** | draft-core, draft-spec, draft-arch, plan-assist, test-assist, research-team, full-auto-team, load-issues, load-plans |
| **sonnet** | gen-arch, deploy |
...
| コード品質 | `.cod` | 未使用コード、依存関係、構造、リント、品質 | リリース前、リファクタ後 |
...
**Codex版（-dx）**: `.cod-dx` / `.sec-dx` はClaude版と同じスコープをCodex（GPT-5.3）で実行する。異なるモデルの視点でダブルチェックしたい時に使う。Claude版の後に実行して結果を突き合わせるのが効果的。

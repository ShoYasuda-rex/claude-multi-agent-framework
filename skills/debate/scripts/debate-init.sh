...
# Output: JSON with file_path + 4 prompts (Claude-A/B/D/V)
...
- テーマ: {theme}
- 参加者: Claude-A, Claude-B, Claude-D, Claude-V
- フェーズ: 発散
- 開始: {datetime_str}
...
## 収束フェーズ（\`## 収束\` 検知で切り替え）
- \`## 収束\` 直後の行で役割が割り当てられる（例: Claude-A=設計者, Claude-B=批評者, Claude-D=実利判定者）
- 順番: 設計者 → 批評者 → 実利判定者 を1ラウンドとする
- 順番判定: 直前の発言者が実利判定者 or User → 設計者の番 / 設計者の後 → 批評者 / 批評者の後 → 実利判定者
- Claude-V(検証者)は収束でも検証専任を継続（順番には入らない）
- V の発言は順番に影響しない
- 設計者: 素材を整理・構造化。全指摘に回答（逃げない）。検証者の指摘も反映。
- 批評者: 穴を突く・エッジケース・実現可能性を問う。対案は出さない（設計者の仕事）。
- 実利判定者: 提案の必要性・ROIを問う。現状維持と比べてコストに見合うか判定。不要なものは「不要」と明言。
- Round 1: 設計者は発散の素材整理から（自分の案ではなく）。対立意見も両方提示し方針提案。Claude-Dのゼロベース視点も素材として拾うこと。
- Round 2〜: 設計者は批評者指摘への修正or反論 + 実利判定者の「不要」判定への反論or受け入れを全て回答。
...
## あなたの役割
- **発散フェーズ:** 既存の前提・制約・現行の仕組みを全部外して「ゼロから作るなら？」「そもそもこれ要るのか？」の視点で発言する。他のエージェント（A/B）の議論の流れに乗らず、根本的に違うアプローチを投げ込む。
- **収束フェーズ:** 実利判定者として参加。提案の必要性・ROI・現状維持との比較を問う。不要なものは「不要」と明言する。

## 動作ループ
1. ファイルを Read で読む
2. \`## 結論\` があれば終了
3. \`## 収束\` がない → 発散モード: 直前の発言者が自分以外なら、\`### [Claude-D]\` + 2〜3行の短文を Edit でファイル末尾に追記
4. \`## 収束\` がある → 収束モード: 自分の役割（実利判定者）・順番を確認し、自分の番なら1発言を追記
5. sleep 3秒 → 1に戻る

## ルール
- 2〜3行の短文のみ（発散時）。箇条書き・見出し・構造化は禁止。
- 既存のコード・設計・制約を前提としない（発散時）。
- 「もしゼロから作るなら」「そもそも不要では」「全く別のアプローチ」等の切り口で発言する（発散時）。
- 収束では提案の必要性・ROIを問い、不要なら「不要」と明言する。
- 日本語で議論。'''

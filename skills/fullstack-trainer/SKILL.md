...
feature-trainer修了者が「DB + 外部API + サーバーレス」を加えた小さなフルスタックプロダクトを1つ作り切ることで、フルスタック開発の型を身体で覚えるための学習スキル。テーマを変えて繰り返し使える。1回あたり3日で完了する想定。
...
6. /draft-arch ← バックエンド構成が加わる
7. DB設計 ← 新しいPhase。テーブル定義とマイグレーション
8. 計画 → 実装
9. 検証
10. デプロイ
11. 振り返り
...
**ユーザーが「自分でテーマを考えたい」と言った場合**：許可するが、スコープチェックを行う。3日・テーブル2以内・エンドポイント5以内・無料APIの制約に収まるか確認し、収まらなければ縮小を提案する。
...
.spc → 仕様適合チェック（CORE.md/ARCHITECTURE.md通りの体験になっているか）

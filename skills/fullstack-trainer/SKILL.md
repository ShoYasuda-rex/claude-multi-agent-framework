...
- **既知のスキル（/kickoff、/verify、/deploy等）を使う場面でも、今回の文脈での役割を毎回説明する。「前回やったから省略」はしない**
...
- **既知のスキル（/kickoff、/verify、/deploy等）の解説を省略する（feature-trainerでやったからと言って飛ばさない）**
...
3. スコープ固定
4. /kickoff ← 「何を作るか」＋「どう作るか」を一気通貫で設計
...
**実行**: /kickoff を起動。fullstack-trainerモードの制約を渡す:
...
- /kickoff は技術が変わっても同じ。「誰の何を解決するか」は普遍
- /verify も同じスキルで、チェック範囲が広がっただけ
- /deploy も同じ。push するだけ。CI/CD が裏で頑張ってくれる

■ 今回使ったスキル（振り返り）
- /kickoff: 「何を作るか」＋「どう作るか」を決めた → docs/CORE.md, docs/ARCHITECTURE.md
...
- /kickoff では Workers + D1 + KV の構成を含める指示を追加する
...
- /kickoff, /verify, /deploy は技術が変わっても同じスキルで対応できること

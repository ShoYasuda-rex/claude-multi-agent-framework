...
フレームワーク初学者が「小さなプロダクト」を1つ作り切ることで、開発フローの型を身体で覚えるための学習スキル。テーマを変えて繰り返し使える。1回あたり1日で完了する想定。
...
| 失敗 | Phase | 対処 |
|------|-------|------|
| スコープ爆発（あれもこれも） | 1, 3 | 「1日で完成が最優先。削ろう」 |
| CORE.mdに技術の話を書く | 4 | 「技術はARCH。ここはWhyだけ」 |
| いきなりコード書きたがる | 5前 | 「まず計画を立ててから」 |
| 検証を飛ばしたがる | 8 | 「動くと正しいは違う」 |
...
【スキル（/xxx）】対話しながら進めるもの。会話の流れを共有する。
  → 一緒に相談しながら設計書を作るイメージ。
  例: /kickoff, /verify, /audit, /deploy

【実装指示】やりたいことを伝えると、AIが自動でタスク分割して並列実装する。
  → 「これを作って」と頼んだら勝手にやってくれるイメージ。

今から使うのは主にスキル側。対話しながら一緒に設計を進める。
実装フェーズになったら実装指示に切り替える。
```
...
**ユーザーが「自分でテーマを考えたい」と言った場合**：許可するが、スコープチェックを行う。1日・フロントエンドのみ・localStorage の制約に収まるか確認し、収まらなければ縮小を提案する。
...
以下のアカウントが必要。持っていなければ一緒に作る（無料）：
- **GitHub アカウント** — https://github.com にアクセスして Sign up。メールアドレスだけで作れる。「コードの保管庫」を使うために必要。
- **Cloudflare アカウント** — https://dash.cloudflare.com/sign-up にアクセスして登録。「作ったサイトをインターネットに公開する」ために必要。

アカウントを作るところからAIがナビゲートする。画面を見ながら一緒に進める。
...
```
/kickoff は「何を作るか」と「どう作るか」を一気通貫で決めるスキル。

前半はCORE.md。技術の話は一切しない。「誰の、何の課題を、どう解決するか」だけ。
後半はARCHITECTURE.md。技術的に「どう作るか」を決める。

なぜ設計を先にやるか

実装中に「これ何のために作ってるんだっけ？」となるのを防ぐ。
AIに実装させる時も、設計ドキュメントがあればAIが判断に迷った時の拠り所になる。
CORE.mdは北極星。ARCHITECTURE.mdは設計図。

やることはシンプル。AIが質問するので、答えるだけ。
「正解」はない。あなたが「こうしたい」と思ったことが正解。

ARCHITECTURE.mdは「設計図」。建物で言えば構造図面。
ここで決めるのは3つ：
...
**実行**: /kickoff を起動する。起動時に以下の制約をコンテキストに含める：

```
【feature-trainer モード】
フレームワーク学習中。対象は1日で作る小さなプロダクト。
テーマ: [選択したテーマ]
技術: [指定した技術スタック]
...
**注意**: 技術スタックは Phase 1 で決まっているので、ARCHITECTURE.md ではディレクトリ構成・コンポーネント設計・localStorageのデータ構造設計に集中させる。
...
ここまでの設計（CORE / ARCH）があるから、AIは正確に作れる。

「こういう機能を作って」と指示すると、
AIが自動でタスクを分割して並列に実装する。
...
実装指示を出す。
「何を作るか」を伝えると、AIが自動でタスクを分割して並列に実装する。

スキルと実装指示の実践的な違い

ここまで（設計フェーズ）はスキルを使って対話しながら一緒に進めた。
ここからは実装指示。タスクを分割して並列で実装する。
...
料理で言えば：
- 味見する（ちゃんと動くか？）→ /verify の静的検証
- 盛り付けを確認する（見た目は大丈夫か？使いやすいか？）→ /verify の動的検証

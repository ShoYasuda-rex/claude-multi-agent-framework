...
#### Safe モード
- `git remote -v` の結果からリモート先を判定し、適切な確認を行う:

**Heroku の場合:**
1. `heroku logs -n 50` でデプロイログを確認（`--tail` は使わない）
2. ログに `error`, `crash`, `H10`, `H12`, `R14` 等のエラーが含まれていないかチェック
3. エラーが検出された場合:
   - エラー内容をユーザーに報告
   - `heroku rollback` でのロールバックを提案し、`AskUserQuestion` で確認を取る
   - 承認された場合、`heroku rollback` を実行
   - ロールバック後、再度 `heroku logs -n 20` でエラーが解消されたか確認
   - 結果をユーザーに報告
4. エラーがなければ成功を報告
